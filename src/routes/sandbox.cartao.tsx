...
export const Route = createFileRoute('/sandbox/cartao')({
...
function TesteSimulacao() {
  ...
  // O document.referrer nos dá a página anterior, o que é ótimo para auditoria
  ...
  const executarFluxo = async () => {
    ...
    const payload = {
      ...
      interaction_context: {
        ...
        utm_campaign: params.get('utm_campaign') || params.get('offer_id') || 'direct_access',
        source_url: window.location.href 
      },
      // Origin: De onde ele clicou (Home)
      origin_url: previousUrl.includes('/sandbox') ? previousUrl : currentUrl, 
      // Target: Onde ele quer chegar (A página atual de Veículos/Cartão)
    ...
    const GATEWAY_URL = `${supabaseUrl}/functions/v1/orchestrator`;
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.details || result.error || "Erro no gateway");
      if (result.action === "REDIRECT") window.location.href = result.url;
    } catch (err: any) {
  ...
  return (
    <div className="p-10 max-w-2xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 p-6 bg-white border border-gray-200 rounded-xl shadow-md">
        ...
        >
          {loading ? "Processando..." : "Disparar Teste de Roteamento"}
        </button>
