...
export const Route = createFileRoute("/sandbox/veiculos")({
      ...
      try {
        ...
        const payload = {
          ...
          origin_visit_update_id:
            params.get("visit_update_id") || sessionStorage.getItem("sbx_last_update_id") || undefined,
...
const executarFluxo = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.details || result.error || "Erro na orquestração");

      if (result.visit_id) {
        // Atualiza os metadados de debug na tela para conferência técnica
        setDebugInfo({
          visitId: result.visit_id,
          updateId: result.visit_update_id || null,
          productId: result.product_id || undefined,
          simulationId: result.simulation_id || null,
          ref: form.offer_id,
        });

        // Captura a URL dinâmica que o banco de dados associou a esta categoria
        const targetPageUrl = result.page_configs?.page_url || result.url;

        if (targetPageUrl) {
          const parsedPath = new URL(targetPageUrl).pathname;
          
          // Redireciona via roteador injetando os parâmetros gerados pelo banco
          navigate({
            to: parsedPath,
            search: {
              visit_id: result.visit_id,
              visit_update_id: result.visit_update_id || undefined,
              simulation_id: result.simulation_id || undefined,
              utm_source: urlParams.get("utm_source") || undefined,
              offer_id: form.offer_id || undefined,
            },
          });
        } else {
          console.error("[ORCHESTRATOR-ERROR] Banco de dados não retornou 'page_url' para esta categoria.");
          alert("Erro de Configuração: Nenhuma URL de destino cadastrada para esta categoria no orchestrator_configs.");
        }
      }
    } catch (err: any) {
