...
function SimulationsPage() {
    ...
    async function loadDropdowns() {
      ...
      if (pData) {
        if (backofficeUser.role === "viewer") {
          ...
          if (allowedPartners.includes("*")) setPartnersList(pData);
          else setPartnersList(pData.filter((p) => allowedPartners.includes(String(p.id))));
        } else {
      ...
      if (prData) {
        if (backofficeUser.role === "viewer") {
          ...
          if (allowedProducts.includes("*")) setProductsList(prData);
          else setProductsList(prData.filter((pr) => allowedProducts.includes(String(pr.id))));
        } else {
  ...
  async function load(targetPage: number) {
    ...
    try {
      ...
      let query = supabase.from("simulations").select(`
          ...
          partners(id, name, logo_url), product_types(id, name),
          stage_types(id, name), status_types(id, name),
      ...
      if (backofficeUser && backofficeUser.role === "viewer") {
        ...
        if (!allowedPartners.includes("*")) {
          if (allowedPartners.length === 0) { setRows([]); setTotalPages(0); setLoading(false); return; }
          query = query.in("partner_id", allowedPartners.map((id: string) => (isNaN(Number(id)) ? id : Number(id))));
        }

        if (!allowedProducts.includes("*")) {
          if (allowedProducts.length === 0) { setRows([]); setTotalPages(0); setLoading(false); return; }
          query = query.in("product_id", allowedProducts.map((id: string) => (isNaN(Number(id)) ? id : Number(id))));
        }
      }
      ...
      if (!simData || simData.length === 0) { setRows([]); setTotalPages(targetPage + 1); return; }
  ...
  async function loadStats() {
    ...
    if (error) { console.error("Erro ao carregar estatísticas de simulações:", error); return; }
      ...
      const { data: fullData, error } = await supabase
        ...
        .select(
          [
            "id", "created_at", "updated_at", "name", "document", "phone", "email",
            "financed_amount", "installment_value", "installments", "down_payment_percentage",
            "raw_payload", "entity_details", "birth_date", "gender", "entity_type",
            "requested_value", "cet_rate", "simulation_details",
            "partners(id, name, logo_url)", "product_types(id, name)", "stage_types(id, name)",
            "status_types(id, name)", "financial_institutions(id, name, logo_url)", "result_partner_types(id, description)",
  ...
  const handleExportExcel = async () => {
    ...
    const dataToExport = filtered.map((sim) => {
      ...
      const rawOffer = safeArray(sim.simulation_offers);
      const offerRow = { ...rawOffer[0], category_types: safeArray(rawOffer[0]?.category_types)[0] || null };
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {filtered.map((r) => {
                ...
                const rawOffers = safeArray(r.simulation_offers);
        ...
        <SheetContent className="w-full sm:max-w-xl flex flex-col h-full p-0 overflow-hidden bg-white">
          {activeSimulation &&
            (() => {
              ...
              const rawOffers = safeArray(sim.simulation_offers);
              const baseOffer = rawOffers[0] || {};
              const offerRow = { ...baseOffer, category_types: safeArray(baseOffer.category_types)[0] || null };
              ...
              const pageConfigs = rawPayloadObj.page_configs || null;
              const pageFaqs = safeArray(rawPayloadObj.page_faqs);
              const extractedConsentConfigs = safeArray(rawPayloadObj.consent_configs).filter(
                (c: any) => c && Object.keys(c).length > 0
              );
              const validConsents = safeArray(sim.simulation_consents).filter((c: any) => c && c.consent_id != null);
            ...
            (() => {
              ...
              return (
                <div className="space-y-6">
                  ...
                  {pageConfigs?.footer && <div className="pt-2 break-inside-avoid"><PanelFooter footer={pageConfigs.footer} /></div>}
                </div>
              );
            })()}
        </div>
