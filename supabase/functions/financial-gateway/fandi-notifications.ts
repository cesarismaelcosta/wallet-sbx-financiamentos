...
export function generateUserEmailNotificationHtml(
  ...
  // Extrai a cor da marca da mesma forma que o cartão
  ...
  // Identificação do Evento e Lote
  ...
  /**
   * Motor de renderização dinâmica do Footer (Padrão mantido)
   */
  const renderFooter = () => {
  ...
  // 2. Montagem do Bloco de Financiamento (Destaque Veículos)
  // Pegamos a primeira consulta (Aprovada)
  const mainConsult = consults && consults.length > 0 ? consults[0] : null;
  let htmlFinanciamento = "";

  if (mainConsult) {
    const parcelas = mainConsult.installments;
    const valorParcela = formatCurrency(mainConsult.installment_value);
  ...
  // 3. Lógica do Botão WhatsApp (Espelho exato do componente React)
  ...
  if (contact) {
    ...
    // Mensagem idêntica à do buildMsg() no React
    ...
    const whatsappUrl = contact.startsWith("http")
      ? `${contact}?text=${encodeURIComponent(msg)}`
      : `https://wa.me/${numericContact}?text=${encodeURIComponent(msg)}`;

    // Renderiza o botão no formato "Outline" usando a cor da marca
    htmlWhatsApp = `
            ...
            <tr>
              <td style="padding: 40px 32px;">
                <p style="font-size: 16px; line-height: 1.6; margin: 0 0 24px 0; color: ${slate};">
                  Olá <b>${nomeCliente}</b>,<br>Temos uma proposta pré-aprovada para você.
                </p>
